...
RP2350にはプログラムを保持する内蔵フラッシュがなく、代わりにボード上の@<b>{外部フラッシュメモリ}にプログラムを書き込みます。
そのため、Raspberry Pi Pico 2Wには4MBのフラッシュが搭載されています。
...
また、フラッシュメモリには命令の他にも、プログラムの実行を通して値が変化しない@<b>{定数}も保存されます。
