...
プログラムや定数を格納するフラッシュメモリを「@<b>{本棚}」であるとすると、SRAMは「@<b>{作業机}」であると考えることができます。
本棚より容量は小さいですが、作業時にすぐ使うものを一時的に置いておける場所、というイメージです。

RP2350には520KBのSRAMがあり、バンクという複数の独立した単位に分かれています。
これにより、CPUとペリフェラル（DMAなど）が同時にアクセスしても速度低下が起きにくくなっています。
