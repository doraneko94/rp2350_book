...
RP2350は、Raspberry Pi社が開発した@<b>{MCU（マイクロコントローラ、マイコン）}です。
...
MCU（マイクロコントローラ、マイコン）とは、CPU・RAM・ROM・I/Oポートなどを1つの集積回路（IC）にまとめた極小コンピュータです。
...
書き込まれたプログラムは@<b>{命令}の羅列として保存されます。
...
外部フラッシュメモリ・SRAMともに、メモリ上の情報が配置される場所には@<b>{アドレス}という数値が割り振られ、その名のとおり「情報の番地」として管理されます。

==== ③ CPUが命令を実行

プログラム（＝命令）を実行するのは@<b>{CPU}です。
CPUは@<b>{レジスタ}という独自の小さなメモリ群を持ち、そこに外部フラッシュメモリやSRAMにある情報をコピーしながら命令を処理します。
...
GPIOのように、MCUに搭載され、様々な機能を実現する周辺装置のことを、@<b>{ペリフェラル}といいます。
...
情報を保存する場所を@<b>{メモリ}といいます。

RP2350がプログラムを実行するとき、@<b>{命令}と@<b>{データ}は異なる種類のメモリに置かれます。
Lチカの例では、命令（プログラムの本体）は外部の@<b>{フラッシュメモリ}に、実行中に変化するデータ（変数など）は@<b>{SRAM}に配置されていると説明しました。
...
それぞれの性質と用途をまとめると、@<table>{tbl-memory}のようになります。
...
ブートROM	起動時の制御コード	極小	消えない（不揮発）
//}

つまり、

 * フラッシュメモリ：プログラムを長期的に保存する場所 
 * SRAM：実行中にCPUが利用する一時的な作業領域
 * ブートROM：起動時に最初に動作する制御コード

という役割分担になっています。

=== フラッシュメモリ ― 命令の「保管庫」

RP2350にはプログラムを保存するためのメモリが内蔵されておらず、代わりにボード上の@<b>{外部フラッシュメモリ}にプログラムを書き込みます。
本書で使用する@<b>{Raspberry Pi Pico 2W}のボード上には、4MBのフラッシュメモリが搭載されています@<fn>{fn-0}。

//footnote[fn-0][RP2354という派生チップは、プログラム保存用のフラッシュメモリをチップ内に積層した構成になっています。]

RP2350内部のSRAMにプログラムを直接書き込まない理由は、メモリの@<b>{性質とコスト構造}にあります。

 * SRAMは非常に高速だが容量が小さく、電源を切ると内容が消える。プログラムを保持するには不向き。
 * フラッシュメモリは書き換えは遅いが、電源を切っても消えず、安価に大容量を確保できる。

SRAMのように、電源を切ると情報が消えてしまう性質を@<b>{揮発性}といいます。
それに対し、電源を切っても情報が消えないフラッシュメモリは@<b>{不揮発性}です。

容量の面でも、RP2350が内蔵するSRAMは520KBしかありません。
これでは、多くのアプリケーションに必要な数百KB〜数MB規模のプログラムを格納するには容量が不足します。

したがって、命令を長期間保持できるフラッシュメモリの外付けが必要となります。

====[notoc] 定数の保存

命令の他にも、プログラムの実行を通して値が変化しない@<b>{定数}（@<list>{const}）もフラッシュメモリに保存されます。
...
SRAM（Static RAM）は揮発性ですが、高速に読み書きでき、CPUが最も頻繁にアクセスするメモリです。
命令やデータを「本」、フラッシュメモリを「@<b>{本棚}」であるとすると、SRAMは「@<b>{作業机}」であると考えることができます。
本棚より容量は小さいですが、作業時にすぐ使う数冊だけを一時的に置いておける場所、というイメージです。

RP2350には520KBのSRAMがあり、バンクという複数の独立した単位に分かれています。
これにより、CPUとペリフェラル（DMA@<fn>{fn-1}など）が同時にアクセスしても速度低下が起きにくくなっています。

//footnote[fn-1][DMAについては後述します。]

SRAMには次のような情報が配置されます。

 * @<b>{変数}や@<b>{配列}など、プログラム実行中に変化するデータ
 * @<b>{スタックフレーム}（関数呼び出し時の一時領域）
 * @<b>{DMAバッファ}など、ペリフェラルが利用する一時的データ

Lチカの例では、@<code>{led}変数などの制御情報がSRAMに確保され、CPUはその値を参照しながらGPIOレジスタへ出力を行います。

====[notoc] スタック ― 一時的な情報を積み上げる場所



=== ROMとブートROM ― 起動を支える小さな制御プログラム

RP2350のチップ内部には、書き換えできないROM領域が存在します。
この領域は製造段階で固定されたプログラムを含み、電源投入直後にCPUが最初に実行します。

とくに重要なのが@<b>{ブートROM}です。
ブートROMは、RP2350が起動した瞬間に実行される制御プログラムであり、主に次のような処理を担当します。

 * as

====[notoc] QSPIとXIP ― フラッシュメモリ上の命令を直接実行する仕組み

RP2350は、外部フラッシュメモリを@<b>{QSPI}（Quad Serial Peripheral Interface）という高速な通信方式で接続しています。

1本の通信線で、データを1ビットずつ順番に送る通信方式を@<b>{シリアル通信}といいます。
代表的なものに@<b>{SPI}方式がありますが、QSPIはその改良版で、4本のデータ線を使用して並列に通信を行うので高速なデータ転送が可能です。

QSPIの重要な特徴は、外部フラッシュメモリをCPUの@<b>{アドレス空間}に直接マッピング（配置）できることです。
この仕組みにより、CPUは、外部フラッシュメモリ上の命令を内部SRAM上にあるかのように読み出して実行できます。
...
その結果、@<b>{起動が高速化され、SRAMを実行時データのみに使える}という利点があります。
...
RP2350が採用するXIP構成は、@<b>{外部フラッシュの大容量をそのまま活かし、SRAMを動的な処理専用に温存できる}という点で合理的です。
